...
class StabilityImportantTest:
    ...
    STABLE_SUMMARY_TRASHOLD = 1
    GOOD_PERFORMANCE_TRASHOLD = 0.3
    ...
    def run(self):
        ...
        for dataset_name, dataset in StabilityImportantTest.DATASETS.items():
            ...
            for metric_name, metric in StabilityImportantTest.METRICS.items():
                ...
                while stable_summary is None or unstable_summary is None:
                    ...
                    if converge_report.total_score < StabilityImportantTest.GOOD_PERFORMANCE_TRASHOLD:
                        # if it is stable
                        stability = self._stability_test(dataset=dataset,
                                                         ...
                                                         max_iter=StabilityImportantTest.MAX_ITER)
                        if stability < StabilityImportantTest.STABLE_SUMMARY_TRASHOLD and stable_summary is not None:
                            stable_summary = summary
                            answer[metric_name]["stable_score"] = stability
                        elif stability > StabilityImportantTest.STABLE_SUMMARY_TRASHOLD and unstable_summary is not None:
    ...
    def _add_dist_lines_to_db(self,
                              db: pd.DataFrame):
        ...
        # sample them once
        answer = [for col in list(db)]
        return db

    def _add_random_lines_to_db(self,
                                db: pd.DataFrame):
        db.append()
