...
class PaperPlots:
    ...
    def run():
        ...
        #PaperPlots.heatmap_one_d_new()
        #PaperPlots.heatmap()
        PaperPlots.skyline()

    @staticmethod
    def skyline():
        df = pd.read_csv(os.path.join(os.path.dirname(__file__), "skyline_data.txt"))
        x = df["reduction_in_time"]
        y = df["reduction_in_acc"]
        ax = plt.subplot()
        plt.scatter(x,
                    y,
                    marker="o",
        ...
        plt.yticks([i*0.005 for i in range(11)])
    ...
    def heatmap_one_d_new():
        ...
        ax1.set_ylabel("Average reduction in computation time")
        plt.grid(alpha=0.1,
                 color="black")
        ...
        ax2.errorbar(x=x,
                     ...
                     markersize=6,
                     capsize=2)
        ax2.set_ylabel("Average reduction in accuracy")
        ax2.set_ylim((0,1))
        plt.tight_layout()
        fig.legend()
        plt.savefig("samples_line_plot.png")
        plt.close()
        ...
        plt.tight_layout()
        fig.legend()
        plt.savefig("features_line_plot.png")
        plt.close()
