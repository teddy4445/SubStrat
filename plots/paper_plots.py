...
class PaperPlots:
    ...
    def heatmap():
        acc_data = [[0.42, 0.43, 0.43, 0.43, 0.58, 0.58, 0.58, 0.58],
                    ...
                    [0.43, 0.43, 0.43, 0.43, 0.58, 0.70, 0.70, 0.70],
                    [0.43, 0.43, 0.43, 0.43, 0.58, 0.70, 0.70, 0.70]]
        acc_data = [[val/0.7 for val in row]for row in acc_data]
        ...
        time_data = [[val/88.46 for val in row]for row in time_data]
        ...
        ax = sns.heatmap(df, vmin=0, vmax=1, cmap="coolwarm")
        ax.xaxis.tick_top()
        ax.xaxis.set_label_position('top')
        plt.ylabel("Rows")
        plt.xlabel("Columns")
    ...
    def ga_generation_plot():
        ...
        :return:
        """
        df = {"GA save time ratio": [21.47, 14.15, 10.34, 08.46, 06.62, 05.89, 05.09, 04.56, 04.15, 03.59, 03.26, 2.96],
              "GA accuracy":        [0.135, 0.101, 0.087, 0.043, 0.026, 0.015, 0.008, 0.008, 0.005, 0.003, 0.002, 0.002],
              "MAB save time ratio": [27.47, 13.84, 10.89, 08.43, 05.26, 04.12, 03.48, 02.87],
              "MAB accuracy":        [0.102, 0.068, 0.049, 0.038, 0.033, 0.033, 0.033, 0.032]}
        ax = plt.subplot(111)
